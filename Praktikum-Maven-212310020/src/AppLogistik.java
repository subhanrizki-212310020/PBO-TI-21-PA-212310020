    ...
    private void InputPesanan() {
        ...
        while(!is_exit_input_pesanan) {
            while(i < title_pesanan.length) {
                String values = JOptionPane.showInputDialog(null,"Masukkan " + title_pesanan[i] + ":","Input " + title_pesanan[i],
        ...
        if(pesanan.size() > i) {
            while(!is_exit_lacak_pesanan) {
                ...
                if((input_resi != null) && (input_resi.equals(pesanan.get(3)))) {
                    ...
                    int pilihan = JOptionPane.showConfirmDialog(null, "Apakah Anda ingin keluar ke menu utama?",
